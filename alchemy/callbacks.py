__all__ = ['get_sells_price_and_count', 'get_booking_price_and_count', 'get_target_leads_count']
...
def get_sells_price_and_count(date_from: date = date(2000, 1, 1),
    ...
    with get_session().begin() as session:
        return session.query(func.count(Deals.id),
               ...
               filter(Deals.status_modified_date >= date_from,
                      ...
                      Deals.status == 105,
                      Deals.is_payed_reserve == int(booking_paid)).all()
